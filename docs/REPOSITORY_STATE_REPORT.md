...
## 2026-07-05 Reconnaissance Notes

- The previous technical design report is stale in one major area: the dashboard dynamic routes are no longer empty. `site/[id]`, `post/[id]`, and `band/[id]` all contain page/layout/loading/not-found files.
- The `site/[id]` route is the real site management surface. It lists posts for a site and links to settings and analytics.
- The `post/[id]` route is the real post editor surface. `band/[id]/page.tsx` currently mirrors the post editor shape and does not yet function as a band overview/member management surface.
- The settings UI is split into General, Domains, and Appearance tabs. Domains and Appearance exist and are protected by the parent settings layout.
- GitHub work has moved to the `gh` CLI instead of a custom GitHub MCP server. Open Epic 1 issues are tracked in GitHub as issues #3 through #11.
...
| Site editing (name, description, bio) | **Complete.** Settings page has forms wired to `updateSite`, including a Bio form that writes to `Site.bio`. | `app/app/(dashboard)/site/[id]/settings/page.tsx` — Band Name, Name, Description, Bio forms. Bio form uses `name: "bio"` and `defaultValue: data?.bio`. | High | None |
...
| Branding fields (logo, cover, font) | **Complete for basic branding.** Fields exist on Site model and are exposed in the Appearance settings tab. | `prisma/schema.prisma::Site` — `logo`, `image`, `imageBlurhash`, `font` fields. `app/app/(dashboard)/site/[id]/settings/appearance/page.tsx` — thumbnail image, logo, featured embed, font, and 404 message forms. | High | Labels could be more band-friendly, but controls exist |
| Basic settings | **Complete.** General, Domains, and Appearance tabs exist for core site configuration. | `app/app/(dashboard)/site/[id]/settings/page.tsx`, `domains/page.tsx`, `appearance/page.tsx`, and parent `settings/layout.tsx`. | High | None |
...
| Initial publishing | **Partially complete.** Posts can be created, edited, published, and unpublished through the editor. UI exists, but full end-to-end behavior has not been manually verified. | `Post.published` boolean field in schema. `components/editor.tsx` has a Publish/Unpublish button using `updatePostMetadata(formData, post.id, "published")`. | Medium | Needs manual/E2E verification that public pages show only published posts |
...
| Rich text editor | **Complete.** Novel MDX editor component wired to post edit page. | `components/editor.tsx` — Novel editor. `app/app/(dashboard)/post/[id]/page.tsx` — renders `<Editor post={data}>`. `band/[id]/page.tsx` currently also mirrors this editor page. | High | `band/[id]` naming is misleading and should be clarified or removed |
...
| Publishing workflow | **Partially complete.** `Post.published` boolean exists and the editor exposes Publish/Unpublish. Draft management remains binary. | `prisma/schema.prisma::Post.published` — Boolean, default false. `components/editor.tsx` — Publish/Unpublish button. No `status` enum. | High | No scheduled publishing, version history, or dedicated publishing workflow |
| Draft management | **Partially complete.** Binary draft/published state via boolean. Post cards visibly mark drafts. No draft listing/filter. | `Post.published` boolean. `components/post-card.tsx` renders a "Draft" badge when `!data.published`. | High | No dedicated draft view or draft count in dashboard stats |
...
| Item | Current State | Evidence | Confidence | Gap |
|------|--------------|----------|------------|-----|
| Tenant homepage | **Partially complete.** Shows cover image, blog post links, and embedded content iframe. It still omits bio, social links, and richer band-specific sections. | `app/[domain]/page.tsx` — renders cover `<Image>`, post title links, and `<Iframe url={data.featuredEmbed}>`. | High | Bio/social links are not rendered; layout is still blog-first |
...
| Bio | **Partially complete.** `Site.bio` field exists in schema and can be edited from site settings. No UI displays bio on public page yet. | `prisma/schema.prisma::Site.bio`. Settings page has bio form with `name="bio"`. Tenant homepage does not render bio. | High | Bio is saved but not displayed publicly |
...
| Subdomain management | **Complete.** Subdomain is provided during site creation and editable in the Domains settings tab. | `components/modal/create-site.tsx` collects `subdomain`; `app/app/(dashboard)/site/[id]/settings/domains/page.tsx` exposes a Subdomain form. | High | No reserved-word validation beyond uniqueness/domain input constraints |
...
| Testing | **Partially complete.** A minimal Node test command exists with a focused regression test for the site Bio form. No full unit, integration, or E2E suite exists yet. | `package.json::scripts.test` — `node --test`. `tests/bio-form-regression.test.mjs` checks Bio form wiring. | Medium | Issue #11 still needed for a real onboarding smoke test and broader test strategy |
...
| Analytics | **Partially complete.** Vercel Analytics is mounted globally and events are tracked in several UI flows. Site analytics page is still a mockup. | `app/layout.tsx` renders `<Analytics />`. `components/modal/create-site.tsx`, `components/create-post-button.tsx`, and form/delete components call `va.track()`. `app/app/(dashboard)/site/[id]/analytics/page.tsx` renders `AnalyticsMockup`. | High | No real analytics dashboard data yet |
...
|------|----------|-------------|
| Orphaned Prisma models | `prisma/schema.prisma` | `YouTube`, `Facebook`, and `Instagram` models have no foreign keys, no UI, no code references. `Example` is only tied to MDX example-card behavior, not core band/site functionality. |
...
| Misleading `band/[id]` route | `app/app/(dashboard)/band/[id]/page.tsx` | Route exists but renders the same post editor shape as `post/[id]/page.tsx`; it is not a band overview/member management surface. |
...
|------|---------------|-------------------|-------------------|
| **Epic 1 — First Band Success** | 6 items | 4 items | 0 items |
| **Epic 2 — Band Content** | 3 items | 3 items | 0 items |
| **Epic 3 — Public Band Experience** | 3 items | 4 items | 2 items (performance, accessibility) |
| **Epic 4 — Publishing** | 3 items | 2 items | 1 item (production deployment) |
| **Epic 5 — Platform Completion** | 2 items | 4 items | 2 items (security review, performance tuning) |
| **Epic 6 — Launch Polish** | 0 items | 4 items | 4 items (landing page, responsive audit, accessibility audit, Lighthouse) |

**Total: 17 complete · 21 partial · 9 absent/major gap**
